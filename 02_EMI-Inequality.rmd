...
f_statistics <- c(
  summary(model_ineq0)$fstatistic["value"],
  summary(model_ineq)$fstatistic["value"]
)

f_df1 <- c(
  summary(model_ineq0)$fstatistic["numdf"],
  summary(model_ineq)$fstatistic["numdf"]
)

f_df2 <- c(
  summary(model_ineq0)$fstatistic["dendf"],
  summary(model_ineq)$fstatistic["dendf"]
)
...
row <- tribble(~term, ~m1,  ~m2,
                'F Statistic', formatted_f_statistics[1], formatted_f_statistics[2],
                'F Statistic p value', formatted_f_p_values[1], formatted_f_p_values[2]
)
...
modelsummary(
  models,
  output = "output/EMI-Ineq-LM_summary.html",  # Change to "html" for HTML or "docx" for Word
  ...
  vcov = function(model) vcovHAC(model)
  )

modelsummary(
  models,
  output = "output/EMI-Ineq-LM_summary.tex",  # Change to "html" for HTML or "docx" for Word
  estimate  = "{estimate} [{conf.low}, {conf.high}]",
  statistic = "p={p.value}", conf_level=0.95, fmt=f,
  notes = "Values in square brackets represent 95% confidence intervals.",
  add_rows=row,  gof_omit="AIC|BIC|RMSE|Log.Lik.",
...
f_statistics <- c(
  summary(model_gini)$fstatistic["value"],
  summary(model_ineq)$fstatistic["value"],
  summary(model_ineq8)$fstatistic["value"]
)

f_df1 <- c(
  summary(model_gini)$fstatistic["numdf"],
  summary(model_ineq)$fstatistic["numdf"],
  summary(model_ineq8)$fstatistic["numdf"]
)

f_df2 <- c(
  summary(model_gini)$fstatistic["dendf"],
  summary(model_ineq)$fstatistic["dendf"],
  summary(model_ineq8)$fstatistic["dendf"]
)
...
# List of models
models <- list(
  "INEQ_GINI" = model_gini,
  "INEQ_1912" = model_ineq,
  "INEQ_1912_POL8" = model_ineq8
)

# Generate LaTeX table
f <- function(x) format(x, digits = 3, nsmall = 2)
row <- tribble(~term, ~m1,  ~m2, ~m3,
                'F Statistic', formatted_f_statistics[1], formatted_f_statistics[2],  formatted_f_statistics[3],
                'F Statistic p value', formatted_f_p_values[1], formatted_f_p_values[2], formatted_f_p_values[3]
)
                
attr(row, 'position') <- 22

modelsummary(
  models,
  output = "output/EMI-Ineq-extra-LM_summary.html",  # Change to "html" for HTML or "docx" for Word
  estimate  = "{estimate} [{conf.low}, {conf.high}]",
  statistic = "p={p.value}", conf_level=0.95, fmt=f,
  notes = "Values in square brackets represent 95% confidence intervals.",
  add_rows=row, gof_omit="AIC|BIC|RMSE|Log.Lik.",
...
modelsummary(
  models,
  output = "output/EMI-Ineq-extra-LM_summary.tex",  # Change to "html" for HTML or "docx" for Word
  estimate  = "{estimate} [{conf.low}, {conf.high}]",
  statistic = "p={p.value}", conf_level=0.95, fmt=f,
  notes = "Values in square brackets represent 95% confidence intervals.",
  add_rows=row, gof_omit="AIC|BIC|RMSE|Log.Lik.",
