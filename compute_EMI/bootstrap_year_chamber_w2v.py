...
def config(parser):
    ...
    parser.add_argument('--output_path', type=str, default='../../data/bootstrap_year_chamber_w2v/')
    return parser 
...
def main(args):
    ...
    grouped_df = chunked_filtered_df.groupby(['starting_year', 'chamber'])
    ...
    for name, group in tqdm(grouped_df):
        ...
        df[['starting_year', 'chamber']] = [name[0], name[1]]
        df.to_csv(Path(args.output_path, f'{name[0]}_{name[1]}.csv', index=False)
